        ...
        <tbody>
          {students.map((student) => (
            <tr key={student.id} className="border-b-2 border-gray-300">
              ...
              <td className="text-gray-600 py-2 px-4 text-center">{formatDate(student.birthdate)}</td>
              <td className="text-gray-600 py-2 px-4 text-center">{student.age}</td>
              <td className="flex justify-center py-2 px-4">
                <button className="bg-blue-500 text-white rounded py-1 px-2 mr-2" onClick={() => handleUpdate(student.id)}>Update</button>
                <button className="bg-green-500 text-white rounded py-1 px-2 mr-2" onClick={() => handleSeeMore(student.id)}>See More</button>
                <button className="bg-red-500 text-white rounded py-1 px-2" onClick={() => handleDelete(student.id)}>Delete</button>
              </td>
            </tr>
          ))}
        </tbody>
